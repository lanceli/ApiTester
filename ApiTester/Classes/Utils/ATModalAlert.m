...
+ (void) say: (id)formatstring,...
{
	...
	va_end(arglist);
	[ATModalAlert ask:statement withCancel:@"Okay" withButtons:nil];
	[statement release];
}
...
+ (BOOL) ask: (id)formatstring,...
{
	...
	id statement = [[NSString alloc] initWithFormat:formatstring arguments:arglist];
	va_end(arglist);
	BOOL answer = ([ATModalAlert ask:statement withCancel:nil withButtons:[NSArray arrayWithObjects:@"Yes", @"No", nil]] == 0);
	[statement release];
	return answer;
}
...
+ (BOOL) confirm: (id)formatstring,...
{
	...
	id statement = [[NSString alloc] initWithFormat:formatstring arguments:arglist];
	va_end(arglist);
	BOOL answer = [ATModalAlert ask:statement withCancel:@"Cancel" withButtons:[NSArray arrayWithObject:@"OK"]];
	[statement release];
	return	answer;
}
...
{
	return [ATModalAlert textQueryWith:question prompt:prompt button1:@"Cancel" button2:@"OK"];
}
@end
